...
public class Vec3f extends IVec3<Float, Vec3f> {
    ...
    @Override
    public @NotNull Vec3f subScalar(@NotNull Float val) {
        return new Vec3f(x - val, y - val, z - val);
    }
    ...
    @Override
    public @NotNull Vec3f div(@NotNull Vec3f vec) {
        return new Vec3f(x / vec.x, y / vec.y, z / vec.z);
    }

    @Override
    public @NotNull Vec3f div(@NotNull Float @NotNull ... vecArr) {
        if (vecArr.length != NUM_COMPONENTS) throw new IllegalArgumentException("Invalid number of components");
        return new Vec3f(x / vecArr[0], y / vecArr[1], z / vecArr[2]);
    }

    @Override
    public @NotNull Vec3f divScalar(double scale) {
